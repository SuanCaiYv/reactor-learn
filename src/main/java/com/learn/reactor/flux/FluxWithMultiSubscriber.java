...
public class FluxWithMultiSubscriber {
    public static void main(String[] args) {
        ...
        // 设定为当订阅者达到一定数量时，进行发布消息
        Flux<String> publish2 = flux0.subscribeOn(Schedulers.boundedElastic()).publish().autoConnect(3);
        System.out.println("auto-subscribe has been set");
        executorService.submit(() -> {
            System.out.println("haven subscribed 5");
            publish2.subscribe(p1 -> {
                System.out.println("p5: " + p1);
            });
        });
        executorService.submit(() -> {
            System.out.println("haven subscribed 6");
            publish2.subscribe(p2 -> {
                System.out.println("s6: " + p2);
            });
        });
        executorService.submit(() -> {
            System.out.println("haven subscribed 7");
            publish2.subscribe(p3 -> {
                System.out.println("s7: " + p3);
            });
        });
        LockSupport.park();
    }
